    ...
    try {
        const sql = `
            SELECT orders.id, orders.user_id, orders.order_date, orders.status, 
                   JSON_ARRAYAGG(JSON_OBJECT('product_id', orderitems.product_id, 'quantity', orderitems.quantity, 'total_price_kc', orderitems.total_price_kc, 'total_price_eu', orderitems.total_price_eu)) AS items
            FROM orders
